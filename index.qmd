...
our_data = read.csv("C:/Users/curti/OneDrive/Desktop/Data Science MS/Adv Stats Modeling/PCA Project docs/NFL.csv")

summary(our_data)

colSums(is.na(our_data)) #returns missing values of each column

our_data[is.na(our_data)] <- 0

colSums(is.na(our_data)) #returns missing values of each column

#next, only use numerical values
numerical_data <- our_data[,2:51]
head(numerical_data)
data_normalized <- scale(numerical_data)#normalizes data using scale function
head(data_normalized)

corr_matrix <- cor(data_normalized)
ggcorrplot(corr_matrix)

data.pca <- princomp(corr_matrix) #PCA values
summary(data.pca)
#3 variables: StD, prop of var, cum prop of var for each principal component
# prop of var shows how much var is explained by each PC

data.pca$loadings[, 1:2] #this loading matrix explores the PC1 and PC2


fviz_eig(data.pca, addlabels = TRUE) #this is a scree plot of the principal component

fviz_pca_var(data.pca, col.var = "black") #this biplot the impact of each attribute on each PC

fviz_cos2(data.pca, choice = "var", axes = 1:2) #this shows the contribution of each variable in graph

fviz_pca_var(data.pca, col.var = "cos2",
            gradient.cols = c("black", "orange", "green"),
            repel = TRUE) #this shows a combination of the first 2 plots together. A biplot with each variables contribution

#black=low cos2 attributes
#orange=mid cos2 attributes
#green=high cos2 attributes
#better than the other fviz_pca_var graph... just use this

# Find Top n principal component
# which will atleast cover 90 % variance of dimension
which(cumsum(our_data) >= 0.9)[1]

FinalDataSet=Tr(FeatureVector)*Tr(StandardizedOriginalDataSet)
